...
void fini(void)
{
  // ...
	...
	// Convert this part to properly log alloc_total and allo[c_avg
	...
	// log non-deallocated list
	// if no deallocated -> ignore
	if(!is_list_empty(list)) {
		LOG_NONFREED_START();

		// first unfreed mem
		item *current_node = list -> next;

		// if exist unfreed mem
		while(current_node != NULL) {
			// next mem
			
			// logging
			LOG_BLOCK(current_node -> ptr, current_node -> size, current_node -> cnt, current_node -> fname, current_node -> ofs);
...
void *realloc(void *ptr, size_t size) {
	...
	if((find(list, ptr)) != NULL) {
		...
		//before_minus_new = size_before - size;
